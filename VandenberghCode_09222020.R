...
colors<-c("gray","#1F78B4")

figure1<-ggplot(maturations, aes(x=TimeDiff, fill=Takeover.Maturation)) + geom_histogram(color="black", alpha=0.7, breaks=(seq(-1260, 2160, 90)), closed="left") + 
  theme_classic() + xlab("Days relative to male takeover") + scale_fill_manual(values=colors) +
  ylab("Females maturing") + theme(axis.title = element_text(face="bold")) + theme(legend.position = "none") +
  scale_y_continuous(breaks=seq(0, 16, 4), labels=round(seq(0, 16, 4)), limits=c(0,16))
figure1 + scale_x_continuous(breaks=seq(-1080,2160, 360))
...
fit.coxph1 <- coxph(surv_object ~ Takeover.Range + Father.Presence + scale(Cumulative.Rainfall) + scale(Avg.Min) + cluster(ID),  data = survival.data)
summary(fit.coxph1)
...
maturations$Category<-factor(maturations$Category, levels=c("Father absent,\ntakeover in prev. 3 months", "Father absent,\nno takeover in prev. 3 months", "Father present,\ntakeover in prev. 3 months", "Father present,\nno takeover in prev. 3 months"))
...
figure2<-ggplot(data=maturations, aes(x = MaturationAge, y = Category, fill=Category)) + scale_fill_manual(values=colors) + 
  ...
  scale_x_continuous(breaks=seq(3.0, 7, 1)) + geom_vline(xintercept=median(maturations$MaturationAge), lty=3)
...
takeover.summary.age$Post.Takeover<- as.character(takeover.summary.age$Post.Takeover)
...
maturation.summary<-estrogen.maturations %>%
  group_by(Window) %>%
  summarise(Mean=mean(E2.ng.g), SE=std(E2.ng.g), count=n())
maturation.summary$CI<-1.96*maturation.summary$SE

labels<-c(maturation.summary$count)
figureS1<-ggplot(data=maturation.summary, aes(x=Window, y=Mean)) +geom_errorbar(ymin=maturation.summary$Mean-maturation.summary$CI, ymax=maturation.summary$Mean+maturation.summary$CI, width=15)+ geom_point() + geom_line()  + xlab("Days relative to maturation") + ylab("Fecal estrogens (ng/g)") +
  theme_classic() + theme(axis.title = element_text(size=11, face="bold"), axis.text =  element_text(size=10)) + geom_vline(xintercept=0, linetype=2, size=.6) +ylim(0, 10) + xlim(-160,50)
...
figureS2 <- ggplot(data=estimates,
  ...
  xlab('')+ ylab("Hazard Ratio (95% Confidence Interval)") + scale_y_log10(breaks=c(0.5,1,2,4,6)) +
...
## FIGURE S4
## Relationship between age at takeover and likelihood of presumed infanticide
infant.survival<-read.csv("Infant_Survival_Takeover.csv")

## 6-month  cutoff
fit1 <- glm(Died1 ~ Takeover.Age, data=infant.survival, family="binomial")
summary(fit1)

## 3-month cutoff
fit2 <- glm(Died2 ~ Takeover.Age, data=infant.survival, family="binomial")
summary(fit2)

library(visreg)
figS4a<-visreg(fit2, "Takeover.Age", scale="response", line.par = list(col = 'black'), gg=TRUE) + xlab("Age at takeover (years)") + ylab("Death within 3 months?") + theme_classic() + ylim(0,.8)
figS4b<-visreg(fit1, "Takeover.Age", scale="response", line.par = list(col = 'black'), gg=TRUE) + xlab("Age at takeover (years)") + ylab("Death within 6 months?") + theme_classic() + ylim(0,.8)
